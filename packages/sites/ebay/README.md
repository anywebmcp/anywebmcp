...
Watchlist mutation deliberately requires the item and a visible watch control to be mounted in the current item or search page. It clicks the normal eBay watch control and does not call `WatchListAdd`, `WatchListRemove`, or another internal mutation endpoint directly. It refuses to click when the current state is ambiguous, when the user is signed out, or when the item or visible control is not mounted.
...
The implementation keeps shared text, money, URL, challenge, and same-origin request helpers local to the eBay package. Search parsing, item parsing, watchlist reading, and mounted watch-control mutation are separate modules so site-specific selectors and safety rules can be tested independently. Search-card and visible-text fallbacks remain dependent on eBay's current markup.

Run the package's sanitized offline fixtures and tool contract tests with `npm test -w @openwebmcp/site-ebay`. The repository-wide `npm test`, `npm run typecheck`, and `npm run validate:sites` commands include the package in their normal checks. Successful tool behavior and payloads remain unchanged, so this refactor does not require a benchmark backfill.
